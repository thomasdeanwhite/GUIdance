package com.thomasdeanwhite.gui.output;
...
public class TestingStateComparator {
    ...
    public static boolean isSameState(int difference, int size) {
        ...
        return diffPercentage < Properties.TESTING_HISTOGRAM_THRESHOLD;
    }
    ...
    private static Integer[] shrink(Integer[] state){
        Integer[] newState = new Integer[Properties.TESTING_HISTOGRAM_BINS];

        for (int i = 0; i < newState.length; i++) {
            newState[i] = 0;
        }

        float mod = (float) (Properties.TESTING_HISTOGRAM_BINS) / (float) state.length;
    ...
    public static int addState(Integer[] state) {
        if (Properties.TESTING_HISTOGRAM_BINS < state.length) {
            state = shrink(state);
        }
        int closestState = -1;
