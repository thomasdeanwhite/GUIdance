...
truth_name = "truth"
user_name = "user-models"

p_random_name = "Random"
p_detection_name = "Pred"
p_truth_name = "API"
p_user_name = "User Model"
...
load_data <- function(){
  ...
  #r_data$TECHNIQUE = factor(data$TECHNIQUE, levels=c(random_name, detection_name, truth_name, user_name))
  return(data)
}
...
process_data <- function(data){
  ...
  p_data_u = p_data %>% filter(TECHNIQUE==user_name) %>% mutate(Technique = p_user_name)
  
  
  p_data = rbind(p_data_r, p_data_d, p_data_t, p_data_u)
  
  p_data$Technique = factor(p_data$Technique, c(p_random_name, p_detection_name, p_truth_name, p_user_name))
...
get_table_row <- function(){
  names = c('Application', 'Detection Cov.', 'Random Cov.', 'P(Random)', 'A12(Random)', 'Truth Cov.', 'P(Truth)', 'A12(Truth)')
